...
- 居民档案：登记、查询、更新和关联事务。
- 档案合并：重复档案差异预览、逐项字段覆盖、事务关联同事务迁移、旧编号追溯与合并记录留痕。
...
针对同一居民因证件录入差异产生的重复档案，提供受控合并而非删除：

- `GET /api/resident-merges/preview` 先展示两份档案在姓名、证件、联系方式等字段及关联事务上的差异与合并阻碍。
- `POST /api/resident-merges` 在选定主档案后执行合并，`take_from_source` 逐项声明允许从源档案覆盖的字段，`idempotency_key` 保证重复请求安全重放。
- 源档案名下事务在同一事务内迁移到主档案；源档案转为 `merged` 状态，不再受理新业务、不可修改或删除。
- 证件号随主档案时，源档案改留 `MERGED-<id>` 墓碑值，原证件号保存在合并记录中，`GET /api/resident-merges/resolve` 可按旧档案编号或旧证件号追到主档案。
- 并发合并由即时事务与状态检查串行化，合并目标必须是在办档案，链式合并不会成环，被拒绝的合并不产生任何部分迁移。
- 每次合并写入 `resident_merges` 记录，并对字段覆盖、源档案停用和每笔事务迁移分别留下审计事件，可按 `resident-merge:<合并编号>` 关联还原。
